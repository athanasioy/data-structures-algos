...
class ForwardList:
    ...
    class _ListIterator:
        def __init__(self, forwardList):
            self._list = forwardList
            self._current = self._list.first()

        def __next__(self):
            if self._current is None:
                raise StopIteration()
            answer = self._current.element()
            self._current = self._list.after(self._current)
            return answer
    ...
    def __iter__(self):
        ...
        #    yield cursor.value
        #    cursor = cursor.next
        return self._ListIterator(self)
    ...
    def _make_position(self,n) -> Position:
        if n is self._head or n is self._tail: # Avoid making a position with a None pointer to Next
            return None
